...
async function addToThreeLastOfUser(user_id, recipe_id){
    ...
    if (recipe_check.length==0){
        await DButils.execQuery(
            `INSERT INTO historyofuser (user_id,last1) VALUES ('${user_id}','${recipe_id}')`
        );
    }
    //[-,-,-]
    else if(recipe_check[0].last1 == null && recipe_check[0].last2 == null && recipe_check[0].last3 == null){
        await DButils.execQuery(`UPDATE historyofuser SET last1='${recipe_id}',last2='${null}', last3='${null}' WHERE user_id='${user_id}'`);
    }
    //[X,-,-]
    else if(recipe_check[0].last1 != null && recipe_check[0].last1 != recipe_id && recipe_check[0].last2 == null && recipe_check[0].last3 == null){
        await DButils.execQuery(`UPDATE historyofuser SET last1='${recipe_id}',last2='${recipe_check[0].last1}', last3='${null}' WHERE user_id='${user_id}'`);
    }
    //[X,Y,-]
    else if(recipe_check[0].last1 != null && recipe_check[0].last2 != null && recipe_check[0].last3 == null){
...
async function getThreeLastOfUser(user_id){
    ...
    if(recipe_check[0].last1 != null && recipe_check[0].last2 == null && recipe_check[0].last3 == null){
